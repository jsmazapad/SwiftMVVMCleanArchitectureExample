...
class SearchBeersUseCaseTest: XCTestCase {
    ...
    func testSearchBeersUseCaseWhenSuccessFetchingData() {
        ...
        XCTAssertTrue(beersRetrieved != nil, "Beers retrieved are not nil")
        XCTAssertTrue(error == nil, "error is nil")
        XCTAssertTrue(beersRetrieved?.count == 2, "Beers count is ok")
    }
    
    func testSearchBeersUseCaseWhenSuccessAndEmptyFetchingData() {
           
           let expectation = self.expectation(description: "0 beers retrieved")
           expectation.expectedFulfillmentCount = 1
        let beersRepository = BeersRepositoryMock(responseCase: .noResults)
           ...
           XCTAssertTrue(beersRetrieved != nil, "Beers retrieved are not nil")
           XCTAssertTrue(error == nil, "error is nil")
           XCTAssertTrue(beersRetrieved?.count == 0, "Beers count is ok")
       }
    ...
    func testSearchBeersUseCaseWhenFailureFetchingData() {
        ...
        waitForExpectations(timeout: 5, handler: nil)
        XCTAssertTrue(beersRetrieved == nil, "response is nil")
        XCTAssertTrue(error != nil, "error is not nil")
